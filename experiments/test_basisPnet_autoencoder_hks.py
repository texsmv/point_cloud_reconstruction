...
from datasets.Vessels import VesselDataset, VesselDataset_Pset, VesselDataset_HKS1
...
points_dataloader = DataLoader(dataset,batch_size= batch_size, shuffle = False, num_workers = 8, drop_last=True, pin_memory=True)
...
for i, point_data in enumerate(points_dataloader, 0):
    ...
    for i in range (batch_size):
        f1 = F1(X_rec[i], X[i], 0.06)
        c = completitud(X_rec[i], X[i], 0.06)
        p = precision(X_rec[i], X[i], 0.06)
