...
from emd_dist.emd import earth_mover_distance, EarthMoverDistanceFunction, EMDLoss
...
def main(config):
    ...
    reconstruction_loss = EMDLoss().to(device)
    # reconstruction_loss = EMD().to(device)


    #optimization in models parameters
    EG_optim = torch.optim.Adam(chain(E.parameters(), G.parameters()), lr= 0.00001, weight_decay= 0, betas= [0.9, 0.999],amsgrad= False)
    ...
    for epoch in range(starting_epoch, max_epochs ):
        ...
        save_point_cloud(X_cpu, X_rec, epoch, n_fig=1, results_dir=results_dir)
